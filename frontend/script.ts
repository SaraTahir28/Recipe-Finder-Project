...
const backendUrl: string = isLocal
  ? "http://localhost:3000"
...
let cachedRecipes: Record<string, Recipe[]> = {};
let nextPageUrl: string | null = null;
let favoritesDiv: HTMLElement | null = null;

// DOM Elements
const ingredientInput = document.getElementById("ingredient") as HTMLInputElement;
const dietDropDown = document.getElementById("diet") as HTMLSelectElement;
const caloriesFilter = document.getElementById("caloriesFilter") as HTMLSelectElement;
const servingsFilter = document.getElementById("servingsFilter") as HTMLSelectElement;
...
function applyFilters(): void {
  let filtered: Recipe[] = [...results];

  // Calories filter
  const calValue = caloriesFilter.value;
  if (calValue === "under-1000") {
    filtered = filtered.filter(r => r.calories < 1000);
  } else if (calValue === "1000-1500") {
    filtered = filtered.filter(r => r.calories >= 1000 && r.calories <= 1500);
  } else if (calValue === "over-1500") {
    filtered = filtered.filter(r => r.calories > 1500);
  }
  ...
  if (serveValue) {
    ...
    filtered = filtered.filter(r => r.yield === serveNum);
  }
...
// Fetch Recipes
function fetchRecipes(ingredient: string, diet: string): void {
  ...
  const url = `${backendUrl}/recipes?ingredient=${encodeURIComponent(ingredient)}${
    diet ? `&filter=${encodeURIComponent(diet)}` : ""
  }`;
...
// Create Recipe Card
function createRecipeCard(recipe: Recipe, mode: "results" | "favorites" = "results"): HTMLElement {
...
function recipeCard(recipe: Recipe): void {
...
loadMoreBtn.addEventListener("click", () => {
  ...
  const proxyUrl = `${backendUrl}/recipes/next?url=${encodeURIComponent(nextPageUrl)}`;
